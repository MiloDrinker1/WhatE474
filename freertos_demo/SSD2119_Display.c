...
static void LCD_GPIOInit(void){
    ...
    // D2
    RCGCGPIO |= GPIO_PORT_D;                    // activate port D
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_D |= (1<<2);                        // make PD2 an output
    GPIOAFSEL_D &= ~(1<<2);                     // disable alternate functions
    GPIODEN_D |= (1<<2);                        // enable digital I/O on PD2

    // K5
    RCGCGPIO |= GPIO_PORT_K;                    // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_K |= (1<<5);                        // make PK5 an output
    GPIOAFSEL_K &= ~(1<<5);                     // disable alternate functions
    GPIODEN_K |= (1<<5);                        // enable digital I/O on PK5

    // M7
    RCGCGPIO |= GPIO_PORT_M;                    // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_M |= (1<<7);                        // make PM7 an output
    GPIOAFSEL_M &= ~(1<<7);                     // disable alternate functions
    GPIODEN_M |= (1<<7);                        // enable digital I/O on PM7

    // N4, N5
    RCGCGPIO |= GPIO_PORT_N;                    // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_N |= ((1<<4) | (1<<5));             // make PN4, PN5 outputs
    GPIOAFSEL_N &= ~((1<<4) | (1<<5));          // disable alternate functions
    GPIODEN_N |= ((1<<4) | (1<<5));             // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    RCGCGPIO |= GPIO_PORT_P;                    // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_P |= ((1<<0)|(1<<1)|(1<<3)|(1<<4)); // make PP0-1, PP3-4 outputs
    GPIOAFSEL_P &= ~((1<<0)|(1<<1)|(1<<3)|(1<<4)); // disable alternate functions
    GPIODEN_P |= ((1<<0)|(1<<1)|(1<<3)|(1<<4));  // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    RCGCGPIO |= GPIO_PORT_Q;                    // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_Q |= ((1<<0)|(1<<2)|(1<<3));        // make PQ0, PQ2-3 outputs
    GPIOAFSEL_Q &= ~((1<<0)|(1<<2)|(1<<3));     // disable alternate functions
    GPIODEN_Q |= ((1<<0)|(1<<2)|(1<<3));        // enable digital I/O on PQ0, PQ2-3
}
